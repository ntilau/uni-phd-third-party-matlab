function T=RectangleMeshN1a(nx,ny,lx,ly)

% T=RectangleMeshN1a(nx,ny,lx,ly)
%
%   This function creates a regular, nx by ny finite element mesh for
%   the rectangle [0,lx]x[0,ly].  Neumann conditions are assumed on
%   the left edge and Dirichlet conditions on the other three edges.
%   The mesh differs from that produced by RectangleMeshLeftN1 in
%   that the pattern of triangles is symmetric.
%
%   The last three arguments can be omitted; their default values are
%   ny=nx, lx=1, ly=lx.  Thus, the command "T=RectangleMeshLeftN1a(m)"
...
% the triangle-node list and the list of constrained
% nodes:

ii=[1:nx+1,2*(nx+1):nx+1:ny*(nx+1),ny*(nx+1)+1:(ny+1)*(nx+1)];
T=MakeMesh1(nodes,tris,ii);
