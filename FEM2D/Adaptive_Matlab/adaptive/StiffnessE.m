function [K,Ks]=StiffnessE(T,fnk)

% [K,Ks]=StiffnessE(T,fnk)
%
...
%
%   The coefficient k(x,y) is define by fnk, which must
%   be a positive scalar or a function of two variables.
%   If fnk is omitted, k is taken to be the constant
%   function 1. T describes the triangulation of Omega.
%
%   If two outputs are requested, the element stiffness
%   matrices are returned in Ks, an id by id by Nt array
%   (Nt is the number of triangles, id is the number of
%   nodes per element).
%
...
for i=1:Nt
   ...
   % Compute the quadrature nodes on T:
